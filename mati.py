...
def main():
    ...
    opt = argparse.ArgumentParser(epilog="Copyright 2015, Symantec UK Ltd.",description="Convert MATI xml intel")
    opt.add_argument('srcfiles', help='Input xml file or directory')
    opt.add_argument('type', choices=['KFF', 'threatbridge', 'bro', 'ioc','all'], help='Specify the type of the destination')
    opt.add_argument('-d','--dir', help="Optional output dir - default is current dir")
    opt.add_argument('-i','--info', help='Optional threat description associated with the file. For example: Zeus or "Random Animal Campaign"')
    options = opt.parse_args()
    sourcefile = os.path.abspath(options.srcfiles)

    if not os.path.exists(sourcefile):
        print "Unrecognised source path "
        sys.exit(1)
    ...
    if options.dir:
        odir = os.path.abspath(options.dir)
    ...
    if isdir:
        for root,dirs,files in os.walk(sourcefile):
            for x_file in files:
                if x_file.endswith('.xml'):
                    file_convert(os.path.join(root,x_file),info,options.type)

    elif sourcefile.endswith('.xml'):
        file_convert(sourcefile,info,options.type)

    else:
        print "Please ensure source file has the correct file extension"
        sys.exit(1)
